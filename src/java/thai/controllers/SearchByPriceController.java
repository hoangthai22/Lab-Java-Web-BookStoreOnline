...
public class SearchByPriceController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ERROR;
        try {
            String start = request.getParameter("txtPriceStart");
            String end = request.getParameter("txtPriceEnd");
            BookDAO bookDao = new BookDAO();
            List<BookObj> bookList = bookDao.findByPrice(Float.parseFloat(start), Float.parseFloat(end));
            request.setAttribute("bookList", bookList);
            request.setAttribute("keySearch", start + "$ to " + end + "$");
            url = HOME_PAGE;
        } catch (Exception e) {
            LOGGER.info("ERROR at SearchByPriceController:" + e.getMessage());
        } finally {
