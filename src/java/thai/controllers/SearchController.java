...
public class SearchController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<BookObj> bookList = null;

            if (txtSearchByName != null) {
                bookList = bookDao.findBook(txtSearchByName, "");
                request.setAttribute("keySearch", txtSearchByName);
            } else if (categoryName != null) {
                bookList = bookDao.findBook("", categoryName);
                request.setAttribute("keySearch", categoryName);
            }
            request.setAttribute("bookList", bookList);
            url = HOME_PAGE;
        } catch (Exception e) {
            LOGGER.info("ERROR at SearchController:" + e.getMessage());
        } finally {
