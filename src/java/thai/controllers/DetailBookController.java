 ...
 * @author ASUS
 */
@WebServlet(name = "DetailBookController", urlPatterns = {"/DetailBookController"})
public class DetailBookController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (idError == null) {
                BookObj bookDetail = bookDao.getBookById(id);
                request.setAttribute("bookDetail", bookDetail);
            } else {
                System.out.println("idError:" + idError);
                BookObj bookDetail = bookDao.getBookById(idError);
                request.setAttribute("bookDetail", bookDetail);
            }
            url = DETAIL_PAGE;
        } catch (Exception e) {
            LOGGER.info("ERROR at DetailBookController:" + e.getMessage());
        } finally {
